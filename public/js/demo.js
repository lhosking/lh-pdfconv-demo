  ...
  $('.format--list-item-tab').click(function() {
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        //================================================================
        // Code added to process SCHIE referral PDF documents, like those
        // included in '/public/data'.
        //
        // Added code the query the found medical provider in the NPPES 
        // registry.

        var noteText = data.answer_units[3].content[0].text;

        var initProvNameStr = noteText.substring(noteText.indexOf("SIGNED BY ") + 10);
        var endIndex = initProvNameStr.indexOf(" ON ");
        var provNameStr = initProvNameStr.substring(0, endIndex);
        var provNameArr = provNameStr.split(',');
        var provLastName = provNameArr[0].trim().toLowerCase();
        var provFirstName = provNameArr[1].trim().toLowerCase();

        var provTextOutput = "Authoring Provider: " + provFirstName.toUpperCase() + " " + provLastName.toUpperCase();
        var provLinkText = "Provider NPI Registry Lookup: " + provFirstName.toUpperCase() + " " + provLastName.toUpperCase();
        var nppesLink = 'https://npiregistry.cms.hhs.gov/api/?state=CA&postal_code=950*&country_code=US&pretty=on&first_name=' + 
              provFirstName + 
              '&last_name=' + 
              provLastName;

        $('.provider--name').text(provTextOutput);      
        $('.nppes--link').attr('href', nppesLink);
        //================================================================
